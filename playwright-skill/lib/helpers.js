...
/**
 * Connect to a long-running persistent browser daemon.
 * The browser is launched detached on first call and stays alive after the
 * script exits. Subsequent runs reuse the same browser window via CDP.
 *
 * Cookies, localStorage, and sessions live in ~/.playwright-profile.
 * Override with PW_PROFILE_DIR. Override CDP port with PW_DEBUG_PORT (default 9222).
 *
 * Usage:
 *   const { browser, context, page } = await launchPersistent();
 *   // Do work...
 *   await browser.close();   // disconnects only — daemon keeps running
 *
 * The daemon survives until the user closes the browser window manually
 * or runs `helpers.killPersistent()`.
 */
async function launchPersistent(options = {}) {
  ...
  const profileDir = process.env.PW_PROFILE_DIR ||
    path.join(os.homedir(), '.playwright-profile');
  const debugPort = parseInt(process.env.PW_DEBUG_PORT || '9222', 10);

  const checkAlive = () => new Promise((resolve) => {
    const req = http.get({ hostname: '127.0.0.1', port: debugPort, path: '/json/version', family: 4 }, (res) => {
  ...
  if (!alive) {
    console.log(`🚀 Launching persistent browser daemon on port ${debugPort}...`);
    ...
    const args = [
      ...
      `--window-size=${(options.viewport?.width) || 1280},${(options.viewport?.height) || 900}`,
      'about:blank',
    ];
    ...
    if (!alive) {
      throw new Error(`Browser daemon failed to start on port ${debugPort}`);
    }
  } else {
    console.log(`🔗 Reusing persistent browser on port ${debugPort}`);
  }
  ...
  if (envHeaders && Object.keys(envHeaders).length > 0) {
    await context.setExtraHTTPHeaders(envHeaders);
  }

  console.log(`🔐 Profile: ~/.playwright-profile (browser stays open after script exits)`);
  return { browser, context, page };
}

/**
 * Kill the persistent browser daemon. Useful for forcing a clean restart.
 */
async function killPersistent() {
  const http = require('http');
  const debugPort = parseInt(process.env.PW_DEBUG_PORT || '9222', 10);
  ...
  await browser.close();
  // CDP close just disconnects — also signal Chrome to quit via /json/close on each target
  await new Promise((resolve) => {
    const req = http.request({
      hostname: 'localhost', port: debugPort, path: '/json/version', method: 'GET'
    }, () => resolve());
